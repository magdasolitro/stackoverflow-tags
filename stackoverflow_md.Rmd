...
The goal of my analysis is to deduce information about the technologies represented by tags based on how they relate to each other in the network.
...
The nodes assigned to Group 3 correspond to technologies that are commonly used for **Machine Learning applications**. Among these, we find also **R**, the language we used for this project, and **Matlab**, one of the most common alternatives to R.

```{r group3, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}
...
Group 7 is instead composed by only two members, **Angular2** and **TypeScript**: turns out that the former is a language based on latter!
...
We investigate also the similarity between different tags with some global similarity measure and show the results in a correlation plot.
...
We show the 5 most interdisciplinary tags ordering the results by the **Shannon entropy**:

```{r}
x <- arrange(flows, desc(shannon_id)) %>% 
...
```{r power2, echo=FALSE, include=TRUE, warning=FALSE, message=FALSE}
#power = power(AI, 6)[1]

# graph %>%
#     activate(nodes) %>%
#     right_join(tibble(1:62, as_tibble(power)) %>% rename(id = `1:62`, power = vector)) %>%
#     mutate(membership = membership(groups)[name]) %>%
#     activate(edges) %>%
#     mutate(betweenness = centrality_edge_betweenness()) %>%
#     ggraph(layout = 'fr') +
#     geom_edge_arc(aes(alpha = betweenness, colour = as.factor(membership(groups))), strength = 0.05) +
#     geom_node_point(aes(size = power, colour = as.factor(membership(groups)))) +
